...
import gradio as gr
import multipart
from multipart.multipart import MultipartState, CR, LF, HYPHEN, COLON, LOWER_A, LOWER_Z, SPACE, FLAG_PART_BOUNDARY, FLAG_LAST_BOUNDARY
from multipart.exceptions import MultipartParseError
from gradio.components.chatbot import ChatMessage, FileMessage
...
def multipart_internal_write(self, data: bytes, length: int) -> int:
    ...
    self.lookbehind = [b"\x00"[0] for _ in range(len(self.boundary) + 8)]
...
def patch_gradio():
    ...
    blocks.BlockContext.__init__ = init_with_class_name_as_elem_classes(
